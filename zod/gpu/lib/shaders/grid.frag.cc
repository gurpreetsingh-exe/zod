...
vec4 grid(vec3 position, float scale) {
    vec2 coord = position.xy * scale; // use the scale variable to set the distance between the lines
...
float compute_linear_depth(vec3 pos) {
    ...
    float linera_depth = (2.0 * near * far) / (far + near - clip_space_depth * (far - near)); // get linear value between 0.01 and 100
