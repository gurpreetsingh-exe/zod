...
void main() {
  ...
  float visualize_toggle_mask =
      smoothstep(-steprange, steprange, visualize_toggle);
  c = mix(mix(vec3(0.2), toggle_color, float(u_vis == i)), c,
          visualize_toggle_mask);
  float socket_radius = 0.08;
  float socket =
      length(abs(coord) - vec2(0.0f, 0.3f + socket_radius)) - socket_radius;
  ...
  uint extra =
      (uint(1 - visualize_toggle_mask)) | (uint(1 - socket_mask) << socket_bit);
